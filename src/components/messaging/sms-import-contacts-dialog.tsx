...
interface ImportSmsContactsDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onImportComplete: (count: number) => void;
}

export function ImportSmsContactsDialog({ open, onOpenChange, onImportComplete }: ImportSmsContactsDialogProps) {
  ...
  const handleImport = async () => {
    ...
    try {
      const res = await authorizedFetch('/api/sms/import', {
        method: 'POST',
        body: formData,
      });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Upload a CSV file with a 'phone' column.</DialogDescription>
        </DialogHeader>
