...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    setValidationLeads(MOCK_LEADS);
    setAcceptedLeadsCount(MOCK_ACCEPTED_COUNT);
    ...
    <div className="flex h-screen bg-gray-50 font-sans text-gray-800 relative overflow-hidden">
      {isWizardOpen && (
        <AddListingWizard
          ...
          onListingAdded={() => setWizardOpen(false)}
        />
      )}
...
const MOCK_LISTINGS = [
  { id: 1, title: 'Marina View Residences', status: 'Active', created_at: new Date().toISOString() },
  { id: 2, title: 'Downtown Loft', status: 'Draft', created_at: new Date().toISOString() },
];

const MOCK_LEADS = [
  { id: 'lead_1', name: 'Alya Khan', status: 'New', created_at: new Date().toISOString(), source: 'instagram' },
  { id: 'lead_2', name: 'Omar Said', status: 'New', created_at: new Date().toISOString(), source: 'website' },
];

const MOCK_ACCEPTED_COUNT = 4;

const AddListingWizard = ({ onClose, onListingAdded }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
    <div className="w-full max-w-md rounded-2xl bg-white p-6 text-gray-800 shadow-xl">
      <h3 className="text-lg font-bold">Listing Wizard</h3>
      <p className="text-sm text-gray-500">
        Listing creation is disabled in this legacy preview.
      </p>
