...
export async function handleOnboarding(data: any, setBrandColor: (color: string) => void): Promise<SiteTemplate> {
    ...
    let targetDeveloper: string | null = null;
        ...
        dynamicBlocks.push({ 
            ...
            order: order++, 
            data: { 
                welcomeMessage: targetProject ? `Hi! I'm an expert on ${targetProject.name}. Ask me anything about prices or floor plans.` : "Hi! How can I help you find a property in the UAE?",
                focusedProjectId: targetProject?.id || null,
                agentName: "Entrestate AI",
                companyName: data.companyName || "Real Estate Advisory",
                collectLeads: true
            } 
        });
