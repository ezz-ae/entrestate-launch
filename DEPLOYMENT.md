...
- **react-loadable-manifest.json missing:** If Vercel logs `ENOENT: no such file or directory, lstat '/vercel/path0/.next/server/page/react-loadable-manifest.json'`, the new `postbuild` hook runs `node scripts/fix-react-loadable-manifest.mjs` to copy a suitable manifest into `.next/server/page/`.
- **page_client-reference-manifest.js missing:** If Vercel logs `ENOENT: no such file or directory, lstat '/vercel/path0/.next/server/page_client-reference-manifest.js'`, the postbuild hook now runs `node scripts/fix-page-client-reference-manifest.mjs` to copy a suitable app manifest into `.next/server/`.
