...
## Troubleshooting
- **middleware.js.nft.json missing:** Vercel often reports `ENOENT: no such file or directory, open '/vercel/path0/.next/server/middleware.js.nft.json'`. The build now runs `scripts/vercel-artifact-audit.mjs` to log `.next/server`; if the ENOENT persists after these changes, redeploy with **Clear Cache** once instead of patching `.next`.
- **react-loadable-manifest.json missing:** If Vercel logs `ENOENT: no such file or directory, lstat '/vercel/path0/.next/server/page/react-loadable-manifest.json'`, check the audit output to see which manifests exist locally before copying anything manually.
- **page_client-reference-manifest.js missing:** If Vercel logs `ENOENT: no such file or directory, lstat '/vercel/path0/.next/server/page_client-reference-manifest.js'`, use the audit script output to diagnose which artifacts were produced; manual fixes should happen locally, not in the Vercel build step.
