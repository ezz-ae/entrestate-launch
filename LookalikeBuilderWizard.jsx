...
const LookalikeBuilderWizard = ({ onClose }) => {
  ...
  const handleCreate = async () => {
    ...
    setIsCreating(true);
    console.log(`Creating audience: Source=${source}, Size=${sizeMap[size]}`);
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        <label className={`block p-4 border rounded-lg cursor-pointer ${source === 'high_score' ? 'border-indigo-500 bg-indigo-50' : 'border-gray-300'}`}>
          <input type="radio" name="source" value="high_score" checked={source === 'high_score'} onChange={(e) => setSource(e.target.value)} className="hidden" />
        ...
        <label className={`block p-4 border rounded-lg cursor-pointer ${source === 'facebook' ? 'border-indigo-500 bg-indigo-50' : 'border-gray-300'}`}>
          <input type="radio" name="source" value="facebook" checked={source === 'facebook'} onChange={(e) => setSource(e.target.value)} className="hidden" />
    ...
    <div className="space-y-4">
      ...
      <div className="pt-4">
        <div className="flex justify-between mb-2">
          ...
          <span className="font-bold text-indigo-600 text-lg">{sizeMap[size]}</span>
        </div>
        <input type="range" min="0" max="4" step="1" value={size} onChange={(e) => setSize(e.target.value)} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-indigo-600" />
    ...
    <div className="space-y-4">
      ...
      <div className="bg-gray-50 p-4 rounded-lg space-y-3 border border-gray-200">
        ...
        <div className="flex justify-between items-center">
          ...
          <span className="font-medium">{sizeMap[size]} of UAE Population</span>
        </div>
      </div>
