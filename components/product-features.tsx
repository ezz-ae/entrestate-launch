  ...
  return (
    <section className="relative py-20" id="products">
      <div className="pointer-events-none absolute inset-0 opacity-70">
        <div className="absolute left-1/2 top-0 h-[420px] w-[420px] -translate-x-1/2 rounded-full bg-[radial-gradient(circle,rgba(203,181,122,0.15),rgba(0,0,0,0))]" />
      </div>
      <div className="container relative mx-auto px-4">
        ...
        <div className="mt-12 grid gap-8 lg:grid-cols-2">
          {featuredProducts.map((product, index) => (
            <div
              key={product.slug}
              className="group relative overflow-hidden rounded-[32px] border border-white/5 bg-neutral-900/40 p-1 transition-all duration-500 hover:border-[#CBB57A]/30 hover:shadow-[0_0_40px_rgba(203,181,122,0.1)] hover:-translate-y-1 animate-fade-up"
                  ...
                  <ul className="mt-6 space-y-2">
                    {product.highlights.slice(0, 3).map((highlight) => (
                      <li key={highlight} className="flex items-center gap-3 text-sm text-neutral-300">
                        <div className="h-1.5 w-1.5 rounded-full bg-[#CBB57A] shadow-[0_0_8px_rgba(203,181,122,0.6)]" />
                        {highlight}
                      </li>
                    ))}
                  </ul>
